...
if [ -d "$DIR/$LOG_DIR" ]; then
    echo "$LOG_DIR 目录存在"
else
    echo "$LOG_DIR 目录不存在，创建中..."
    mkdir "$DIR/$LOG_DIR"
fi

if [ -r /sys/kernel/debug/kfd/hqds ]; then
    HQD_FILE="$DIR/$LOG_DIR/kfd_${des}_hqds_$(date +%Y%m%d_%H%M%S).txt"
    ...
    # 统计活跃的HQD数量（只取第一行，避免换行导致 integer expression expected）
    active_hqds=$( (grep -c "Active:" "${HQD_FILE}" 2>/dev/null || echo "0") | head -n1)
...
if [ -r /sys/kernel/debug/kfd/mqds ]; then
    MQD_FILE="$DIR/$LOG_DIR/kfd_${des}_mqds_$(date +%Y%m%d_%H%M%S).txt"
    ...
    # 查找ring buffer地址（只取第一行，避免换行导致 integer expression expected）
    ringbuf_count=$( (grep -c "cp_hqd_pq_base" "${MQD_FILE}" 2>/dev/null || echo "0") | head -n1)
...
if [ -r /sys/kernel/debug/kfd/rls ]; then
    RLS_FILE="$DIR/$LOG_DIR/kfd_${des}_rls_$(date +%Y%m%d_%H%M%S).txt"
